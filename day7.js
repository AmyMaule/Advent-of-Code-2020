...
const input = `pale cyan bags contain 2 posh black bags, 4 wavy gold bags, 2 vibrant brown bags.
...
function part1() {
  ...
  bagsArr.forEach(bag => {
    // if one of the child bags is shiny gold, add its parent bag to the set
...
function part2() {
  ...
  while (innerBagsNeeded.length > 0) { // > 0
    if (!innerBagsNeeded[0].match(/other/)) {
      ...
      for (let i = 0; i < innerBagsNeeded[0][0]; i++) {
        ...
        // check bagsArr for the parent bag of currentBag and push its children into innerBagsNeeded
        bagsArr.forEach(bags => {
          if (bags[0].match(currentBag)) {
            bags.forEach((bag, i) => {
              if (i !== 0) {
                innerBagsNeeded.push(bag);
              }
            })
          }
        })
